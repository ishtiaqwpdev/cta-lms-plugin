<?php
/**
 * Validate LMFT Clinical Flashcard Study Center import (PROMPT 05, cards 121–150).
 *
 * Usage: php scripts/test-lmft-clinical-flashcard-import-121-150.php
 *
 * @package CTA_LMS
 */
...
$source_path = CTA_PLUGIN_DIR . 'scripts/_lmft-clinical-flashcards-prompt-05.json';
...
assert_true( count( $stored['cards'] ) >= 150, 'Deck JSON contains at least 150 cumulative cards' );
assert_true( 180 === (int) ( $stored['expected_total'] ?? 0 ), 'expected_total is 180' );
assert_true( (int) ( $stored['imported_through'] ?? 0 ) >= 150, 'imported_through is at least 150' );
...
$chunk_ids = array_map(
	...
	array_slice( $stored['cards'], 120, 30 )
);
assert_true( $expected_ids === $chunk_ids, 'Cards 121–150 match export IDs (D5-011 through D5-040)' );

$sort_orders = array_map(
	static function ( $card ) {
		return (int) ( $card['sort_order'] ?? 0 );
	},
	array_slice( $stored['cards'], 0, 150 )
);
assert_true( range( 1, 150 ) === $sort_orders, 'First 150 sort_order values are 1 through 150 in sequence' );
...
assert_true( (int) ( $deck['count'] ?? 0 ) >= 150, 'Loader count is at least 150' );
...
assert_true( ( $domain_counts['treatment'] ?? 0 ) >= 40, 'Treatment has at least 40 cards' );
...
assert_true( 'D5-040' === $loader_ids[149] || 'D6-018' === $loader_ids[ count( $loader_ids ) - 1 ], 'Final imported card preserved in canonical order' );
...
$first_chunk = $stored_by_id['D5-011'] ?? array();
assert_true(
	strpos( (string) ( $first_chunk['front'] ?? '' ), 'severe dysregulation or dissociation' ) !== false,
	'Card D5-011 front preserved'
);

$last = $stored_by_id['D5-040'] ?? array();
assert_true(
	strpos( (string) ( $last['front'] ?? '' ), 'DBT-informed intervention' ) !== false,
	'Card D5-040 front preserved'
);
