<?php
/**
 * Validate LMFT Clinical Flashcard Study Center import (PROMPT 06, cards 151–180).
 *
 * Usage: php scripts/test-lmft-clinical-flashcard-import-151-180.php
 *
 * @package CTA_LMS
 */
...
function lmft_fsc_expected_domain_key( $domain_code ) {
	switch ( (string) $domain_code ) {
		case 'D5':
			return 'treatment';
		case 'D6':
			return 'managing-legal-and-ethical-obligations';
		default:
			return '';
	}
}

$source_path = CTA_PLUGIN_DIR . 'scripts/_lmft-clinical-flashcards-prompt-06.json';
...
assert_true( 180 === count( $stored['cards'] ), 'Deck JSON has 180 cumulative cards' );
assert_true( 180 === (int) ( $stored['expected_total'] ?? 0 ), 'expected_total is 180' );
assert_true( 180 === (int) ( $stored['imported_through'] ?? 0 ), 'imported_through is 180' );
assert_true( '151-180' === (string) ( $stored['card_range'] ?? '' ), 'card_range is 151-180' );
...
$chunk_ids = array_map(
	...
	array_slice( $stored['cards'], 150, 30 )
);
assert_true( $expected_ids === $chunk_ids, 'Cards 151–180 match export IDs (D5-041 through D6-018)' );
...
assert_true( range( 1, 180 ) === $sort_orders, 'sort_order values are 1 through 180 in sequence' );
...
assert_true( 180 === (int) ( $deck['count'] ?? 0 ), 'Loader count is 180' );
assert_true( 6 === count( (array) ( $deck['domains'] ?? array() ) ), 'Six domains declared' );
...
assert_true( 52 === ( $domain_counts['treatment'] ?? 0 ), 'Treatment has 52 cards' );
assert_true(
	18 === ( $domain_counts['managing-legal-and-ethical-obligations'] ?? 0 ),
	'Managing Legal and Ethical Obligations has 18 cards'
);
...
assert_true( 'D6-018' === $loader_ids[179], 'One hundred eightieth card is D6-018' );
...
$first_d6 = $stored_by_id['D6-001'] ?? array();
assert_true(
	strpos( (string) ( $first_d6['front'] ?? '' ), 'informed consent more than a signature' ) !== false,
	'Card D6-001 front preserved'
);
assert_true(
	'managing-legal-and-ethical-obligations' === (string) ( $first_d6['domain'] ?? '' ),
	'Card D6-001 mapped to Managing Legal and Ethical Obligations domain'
);

$last = $stored_by_id['D6-018'] ?? array();
assert_true(
	strpos( (string) ( $last['front'] ?? '' ), 'employer, court-related source, insurer' ) !== false,
	'Card D6-018 front preserved'
);
