<?php
/**
 * Validate LMFT Clinical Flashcard Study Center import (PROMPT 03, cards 61–90).
 *
 * Usage: php scripts/test-lmft-clinical-flashcard-import-61-90.php
 *
 * @package CTA_LMS
 */
...
$source_path = CTA_PLUGIN_DIR . 'scripts/_lmft-clinical-flashcards-prompt-03.json';
...
assert_true( count( $stored['cards'] ) >= 90, 'Deck JSON contains at least 90 cumulative cards' );
assert_true( 180 === (int) ( $stored['expected_total'] ?? 0 ), 'expected_total is 180' );
assert_true( (int) ( $stored['imported_through'] ?? 0 ) >= 90, 'imported_through is at least 90' );
...
$chunk_ids = array_map(
	...
	array_slice( $stored['cards'], 60, 30 )
);
assert_true( $expected_ids === $chunk_ids, 'Cards 61–90 match export IDs (D2-012 through D4-001)' );

$sort_orders = array_map(
	static function ( $card ) {
		return (int) ( $card['sort_order'] ?? 0 );
	},
	array_slice( $stored['cards'], 0, 90 )
);
assert_true( range( 1, 90 ) === $sort_orders, 'First 90 sort_order values are 1 through 90 in sequence' );
...
assert_true( (int) ( $deck['count'] ?? 0 ) >= 90, 'Loader count is at least 90' );
assert_true( count( (array) ( $deck['domains'] ?? array() ) ) >= 4, 'At least four domains declared' );
...
assert_true(
	...
	'Developing a Diagnostic Impression has at least 20 cards'
);
assert_true(
	( $domain_counts['managing-crisis-situations'] ?? 0 ) >= 20,
	'Managing Crisis Situations has at least 20 cards'
);
assert_true(
	( $domain_counts['case-conceptualization-and-planning'] ?? 0 ) >= 1,
	'Case Conceptualization and Planning has at least 1 card'
);
...
assert_true( 'D4-001' === $loader_ids[89], 'Ninetieth card is D4-001 or later import preserved order' );
...
$first_d3 = $stored_by_id['D3-001'] ?? array();
assert_true(
	strpos( (string) ( $first_d3['front'] ?? '' ), 'acutely intoxicated' ) !== false,
	'Card D3-001 front preserved'
);
assert_true(
	'managing-crisis-situations' === (string) ( $first_d3['domain'] ?? '' ),
	'Card D3-001 mapped to Managing Crisis Situations domain'
);

$first_d4 = $stored_by_id['D4-001'] ?? array();
assert_true(
	strpos( (string) ( $first_d4['front'] ?? '' ), 'treatment goal and a measurable treatment objective' ) !== false,
	'Card D4-001 front preserved'
);
assert_true(
	'case-conceptualization-and-planning' === (string) ( $first_d4['domain'] ?? '' ),
	'Card D4-001 mapped to Case Conceptualization and Planning domain'
);
