<?php
/**
 * Build lmft-clinical-form-b-answer-key-101-125.php from embedded PROMPT 23 JSON.
 *
 * Usage: php scripts/build-lmft-clinical-form-b-answer-key-101-125.php
 *
 * @package CTA_LMS
 */

$root = dirname( __DIR__ );
$json = file_get_contents( __DIR__ . '/_lmft-form-b-admin-prompt-23.json' );
if ( false === $json ) {
	fwrite( STDERR, "Missing scripts/_lmft-form-b-admin-prompt-23.json\n" );
	exit( 1 );
}
...
$out = "<?php\n";
$out .= "/**\n";
$out .= " * ADMIN ONLY — LMFT California Clinical Form B answer key items 101–125 (PROMPT 23).\n";
...
foreach ( $data['records'] as $row ) {
	$num = (int) ( $row['q_num'] ?? 0 );
	$code = 'CTA-LMFT-CA-FB-' . str_pad( (string) $num, 3, '0', STR_PAD_LEFT );
...
$target = $root . '/includes/quiz-seeds/admin-only/lmft-clinical-form-b-answer-key-101-125.php';
