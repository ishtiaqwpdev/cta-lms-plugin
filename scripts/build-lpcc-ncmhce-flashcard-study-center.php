<?php
/**
 * Build LPCC NCMHCE Flashcard Study Center JSON from approved xlsx export.
 *
 * Usage:
 *   php scripts/build-lpcc-ncmhce-flashcard-study-center.php --source=path/to/export.xlsx
 *   php scripts/build-lpcc-ncmhce-flashcard-study-center.php --source=... --dry-run
 *
 * @package CTA_LMS
 */
...
$output  = isset( $options['output'] )
	...
	: $root . '/assets/course-materials/lpcc-ncmhce/study-tools/flashcard-study-center.json';

if ( '' === $source || ! is_readable( $source ) ) {
	fwrite( STDERR, "Usage: php scripts/build-lpcc-ncmhce-flashcard-study-center.php --source=path/to/export.xlsx [--dry-run]\n" );
	exit( 1 );
}
 ...
 */
function lpcc_fsc_read_xlsx_sheet( $path, $sheet_name ) {
	$zip = new ZipArchive();
	...
	foreach ( $sxp->query( '//x:sheetData/x:row' ) as $row ) {
		...
		foreach ( $sxp->query( './x:c', $row ) as $cell ) {
			...
			$col = lpcc_fsc_col_letters_to_index( $m[1] );
			$val = lpcc_fsc_cell_value( $cell, $shared );
			$grid[ $rnum ][ $col ] = $val;
		}
	}
 ...
 */
function lpcc_fsc_col_letters_to_index( $letters ) {
 ...
 * @return string
 */
function lpcc_fsc_cell_value( DOMElement $cell, array $shared ) {
...
try {
	$sheet_rows = lpcc_fsc_read_xlsx_sheet( $source, 'Flashcards' );
} catch ( Throwable $e ) {
